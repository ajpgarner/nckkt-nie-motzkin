function condition = weak_kkt_optimality_condition(obj, states, vIdx, variate)
%WEAK_KKT_OPTIMALITY_CONDITION Generates weak optimality condition.
%
% PARAMS:
...
%
    ...
    if vIdx <= obj.d % Only has support on \bar{a}_i term
        ...
        if ~D_neg_aa.IsZero
            expr = expr - D_neg_aa.yalmip(states.lambda{vIdx});
        end
        if obj.Verbose>=2 
            fprintf(" ∇(1-a_%d^2) = %s\n", ...
